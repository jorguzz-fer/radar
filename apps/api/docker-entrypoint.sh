...
# de subir o servidor NestJS. Roda em todo deploy.
#
# Comportamento:
# - Se DATABASE_URL não está setada, pula a sincronização (deixa o app
#   falhar logo no primeiro request com erro mais claro)
...
if [ "${SKIP_DB_PUSH}" = "1" ]; then
  ...
  echo "[entrypoint] DATABASE_URL não está setada — pulando sync de schema." >&2
else
